...
namespace UiTesting.Source.Input
{
    ...
    public class InputSystem : IDisposable
    {
        ...
        private class InputMouseButton
        {
            ...
            public InputMouseButton(MouseButtons button)
            {
                Button = button;
            }
        }
        ...
        private UiManager userInterface = null;
        private InputOffset inputOffset = new InputOffset();
        ...
        public InputSystem(UiManager userInterface, InputOffset offset)
        {
            this.inputOffset = offset;
            this.userInterface = userInterface;
        }

        public InputSystem(UiManager userInterface) : this(userInterface, new InputOffset(0, 0, 1.0f, 1.0f))
        {
        ...
        public virtual void Initialize()
        {
            ...
            foreach (string str in Enum.GetNames(typeof(MouseButtons)))
            {
                InputMouseButton btn = new InputMouseButton();
                btn.Button = (MouseButtons)Enum.Parse(typeof(MouseButtons), str);
        ...
        public virtual void SendKeybroadState(KeyboardState state, GameTime gameTime)
        {
        ...
        private Point RecalPosition(Point pos)
        {
        ...
        private void BuildMouseEvent(MouseState state, MouseButtons button, ref MouseEventArgs e)
        {
            e.State = state;
            e.Button = button;
            ...
            e.Position = RecalPosition(e.Position);
            ...
            Point pos = RecalPosition(new Point(mouseState.X, mouseState.Y));
            e.Difference = new Point(e.Position.X - pos.X, e.Position.Y - pos.Y);
        }

        private void BuildMouseEvent(MouseState state, MouseButtons button, MouseScrollDirection direction, ref MouseEventArgs e)
        {
        ...
        public void UpdateMouse(MouseState state, GameTime gameTime)
        {
            ...
            if((state.X != mouseState.X) || (state.Y != mouseState.Y))
            {
                ...
                if (state.LeftButton == ButtonState.Pressed) btn = MouseButtons.Left;
                else if (state.RightButton == ButtonState.Pressed) btn = MouseButtons.Right;
                else if (state.MiddleButton == ButtonState.Pressed) btn = MouseButtons.Middle;
                else if (state.XButton1 == ButtonState.Pressed) btn = MouseButtons.XButton1;
                else if (state.XButton2 == ButtonState.Pressed) btn = MouseButtons.XButton2;
        ...
        private void UpdateButtons(MouseState state, GameTime gameTime)
        {
            ...
            foreach (InputMouseButton btn in mouseButtons)
            {
                ...
                if (btn.Button == MouseButtons.Left) bs = state.LeftButton;
                else if (btn.Button == MouseButtons.Right) bs = state.RightButton;
                else if (btn.Button == MouseButtons.Middle) bs = state.MiddleButton;
                else if (btn.Button == MouseButtons.XButton1) bs = state.XButton1;
                else if (btn.Button == MouseButtons.XButton2) bs = state.XButton2;
                else continue;
